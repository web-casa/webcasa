...
func (s *Service) Start() {
	...
	// Subscribe to trigger events from other plugins.
	if s.eventBus != nil {
...
func (s *Service) runCommand(task CronTask) (*CronLog, error) {
	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		// Truncate output (keep the tail, rune-safe).
		outStr := output.String()
		if len(outStr) > maxOutputBytes {
			outStr = outStr[len(outStr)-maxOutputBytes:]
			// Skip to the next valid UTF-8 boundary to avoid cut runes.
			for i, r := range outStr {
				if r != 0xFFFD { // not a replacement character
					outStr = outStr[i:]
					break
				}
			}
