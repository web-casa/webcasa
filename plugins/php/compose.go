...
func GenerateFrankenCompose(site *PHPSite, timezone, customImage string) string {
	image := customImage
	if image == "" {
...
%s    container_name: %s
    ...
    labels:
      webcasa.plugin: php
      webcasa.site: "%s"
`, buildSection, site.ContainerName, site.Port, site.RootPath, timezone, site.Name)
}
...
func GenerateFPMDockerfile(version string, extensions []string) string {
...
func GenerateFrankenDockerfile(version string, extensions []string) string {
...
func GeneratePHPIni(cfg PHPIniConfig) string {
	var lines []string
...
func GenerateFPMPoolConf(cfg FPMPoolConfig) string {
	var lines []string
	lines = append(lines, "; Generated by WebCasa PHP Manager")
