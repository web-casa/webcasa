...
CaddyPanel Phase 6 批量开发，包含 6 个新功能：站点一键克隆、域名 DNS 解析检查、响应式移动端适配、2FA TOTP 双因素认证、站点分组/标签、站点模板。这些功能旨在提升面板的易用性、安全性和管理效率。

## 术语表

- **Panel**：CaddyPanel Web 管理面板（Go 后端 + React 前端）
...
**用户故事：** 作为管理员，我想将常用的站点配置保存为模板并复用，以便快速创建标准化配置的新站点，也能与其他 CaddyPanel 实例共享模板。
