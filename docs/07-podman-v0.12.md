...
| R5 | rootful podman socket 在 SELinux enforce 下 permission denied | 中 | WebCasa 连不上 socket | install.sh 靠 DAC(`podman` 组)即可，完整 SELinux 运维文档见 [`docs/selinux.md`](selinux.md) |
...
| 天 | 任务 | 交付物 |
|----|------|--------|
| 1 | SELinux 运维文档 (R5) → `docs/selinux.md` ✅；storage.conf native overlay (R7) | install.sh 生产 ready |
