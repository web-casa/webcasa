...
| 项目 | 要求 |
|------|------|
| 操作系统 | RHEL 9/10 系列 Linux（amd64 / arm64） |
| 内存 | Lite：最低 256MB，建议 512MB+；Full：最低 1GB，建议 2GB+ |
| 磁盘 | 最低 200MB（不含项目数据） |
| 网络 | 需要开放 80、443 端口（用于 HTTPS） |

说明：Lite 指仅使用核心 Caddy 管理能力；Full 指开启全部插件功能。Docker 部署时无需额外依赖，二进制部署需要自行安装 Caddy。
...
```bash
# CentOS Stream / AlmaLinux / Rocky Linux / Fedora
