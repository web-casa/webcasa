...
func setupTestManager(t *testing.T) (*Manager, *gorm.DB) {
	...
	operatorRg := r.Group("/api/plugins")
	adminRg := r.Group("/api/plugins")
	publicRg := r.Group("/api/plugins")
	dataDir := t.TempDir()
	mgr := NewManager(db, rg, operatorRg, adminRg, publicRg, &stubCoreAPI{}, dataDir)
	return mgr, db
}
...
func (s *stubCoreAPI) DatabaseCreateInstance(req DatabaseCreateInstanceRequest) (uint, error) {
	return 0, nil
}
...
func (s *stubCoreAPI) DockerRunContainer(req DockerRunContainerRequest) (string, error)     { return "", nil }
